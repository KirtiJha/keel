 ...
 * it at the same instant. Two properties are load-bearing and neither is
 * visible from a functional test: reading it must stay linear in the number of
 * records, and folding it must not lose records when two processes fold at once.
 *
 ...
 * journal to 40 s against a 30 s hook timeout. Compaction read the snapshot,
 * folded, and wrote it back with nothing protecting the sequence, so two
 * overlapping compactions each dropped the other's records after the journal
 * they came from had already been deleted.
 */
...
/** Enough journal to cross the compaction threshold, then one ordinary update. */
function forceCompaction(root: string, branch: string): void {
  let bytes = 0;
  let round = 0;
  while (bytes < COMPACT_BYTES) {
    bytes += bulkJournal(root, branch, 400, `filler${round}`);
    round++;
  }
...
describe("reading the ledger stays cheap enough for a hook", () => {
   ...
   * every record it applied, so 4x the records cost ~16x the time. Linear work
   * costs ~4x. The ratio is what distinguishes them; the margin is wide enough
   * that a loaded CI machine cannot turn a linear fold into a failure.
   */
   ...
   * and once more for the RED lookup. Twenty new exports over a populated
   * ledger measured 40 s against a 600 ms budget and a 30 s platform timeout,
   * so the gate reached no verdict at all and the edit appeared to hang.
   */
  it("runs the gates over a populated ledger inside the hook's timeout", async () => {
    ...
    // A snapshot with 800 expectations, and a journal just under the threshold
    // so the run pays the fold without also paying a compaction.
    ...
    let bytes = 0;
    let round = 0;
    while (bytes < COMPACT_BYTES - 20_000) {
      bytes += bulkJournal(repo.root, branch, 400, `j${round}`);
      round++;
    }
 ...
 * gone for good. Eight writers past the threshold reproduced it without any
 * instrumentation: 3200 records in, 2483 out.
 */
describe("compaction under concurrency", () => {
  ...
  function runWriter(writer: string, index: number): Promise<void> {
    return new Promise((resolve, reject) => {
      const child = spawn(
        process.execPath,
        [writer, repo.root, `c${index}`, String(PER_WRITER)],
        { stdio: "ignore" },
      );
  ...
  /**
   * A snapshot that could not be written must leave the rotated journal in
   * place. Writing the snapshot and then deleting the journal it came from in
   * the wrong order turns one failed write into permanent data loss.
   */
  it("keeps every record when the snapshot cannot be written", () => {
    ...
    // rename onto it fails, which is exactly the shape of a full or read-only
    // disk without needing either.
    ...
    let round = 0;
    while (bytes < COMPACT_BYTES) {
      bytes += bulkJournal(repo.root, branch, 400, `fail${round}`);
      round++;
    }
    expect(() => {
...
/**
 * Nothing prunes the ledger in production — the only two functions that could
 * have (`writeState`, `clearBranch`) have no production caller — so it grew
 * without bound and fed the read cost above. Compaction is the one step that
 * rewrites the snapshot, so it is where the bound belongs.
 */
describe("the ledger is self-limiting", () => {
  ...
  it("bounds how many test files one branch retains", () => {
    const branch = currentBranch(repo.root);
    bulkJournal(repo.root, branch, MAX_EXPECTATIONS_PER_BRANCH + 500);
    recordTestWritten(repo.root, "src/newest.test.ts", 9_000_000);

    const kept = branchState(readState(repo.root), branch).expectations;
    expect(Object.keys(kept).length).toBeLessThanOrEqual(MAX_EXPECTATIONS_PER_BRANCH);
  ...
  it("leaves the journal alone rather than losing it when compaction cannot run", () => {
    const branch = currentBranch(repo.root);
    recordTestWritten(repo.root, "src/a.test.ts", 1000);

    forceCompaction(repo.root, branch);
    // Compaction ran: the journal was folded away and the snapshot exists.
